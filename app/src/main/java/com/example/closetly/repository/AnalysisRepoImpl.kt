...
class AnalysisRepoImpl : AnalysisRepo {
    
    override fun calculateActivityStats(clothesList: List<ClothesModel>): ActivityStats {
    ...
    override fun calculateClosetBreakdown(clothesList: List<ClothesModel>): List<ClosetCategory> {
    ...
    override fun calculateMostWornColors(clothesList: List<ClothesModel>): List<WornColor> {
        if (clothesList.isEmpty()) return emptyList()
